...
export default function GrantIdeas() {
		...
		try {
			if (contract && id) {
				...
				setIdeasURI({
					...
					donation: Number((await contract._ideas_uris(Number(id)).call()).donation) / 1e6,
					isVoted: isvoted,
	...
	async function VoteIdees() {
		...
		try {
			await sendTransaction(await window.contract.create_goal_ideas_vote(Number(Goalid), Number(id), signerAddress));
		} catch (error) {
			console.error(error);
			return;
		}
	...
	async function PostComment(e) {
		...
		let newComment = {
			address: window?.tronWeb?.defaultAddress?.base58.toString().toLocaleLowerCase().toString(),
			message: Comment,
	...
	async function saveMessage(newComment) {
		await sendTransaction(await window.contract.sendMsg(Number(ideaId), JSON.stringify(newComment)));
		removeElementFromArrayBYID(emptydata, 0, setemptydata)
	...
	async function sendReply(replyText, MessageId, MessageIndex) {
		...
		await sendTransaction(await window.contract.sendReply(Number(MessageId), JSON.stringify(newReply)));
		removeElementFromArrayBYID(emptydata, 0, setemptydata)
		...
		<>
			...
			<div className={`${styles.container} flex flex-col items-center gap-8`}>
				<div className={`${styles.title} flex flex-col gap-2`}>
					...
					<Loader element={<div className="flex">Donated: {IdeasURI.donation} TRX </div>} width={"100%"} />
